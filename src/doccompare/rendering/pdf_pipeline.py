...
def _render_deletions_html(deletions: list | None) -> str:
    """Render deleted text sections as HTML for the summary page."""
    if not deletions:
        return ""

    items = []
    for d in deletions[:30]:  # Cap at 30 to avoid huge pages
        escaped = html_mod.escape(d)
        if len(escaped) > 200:
            escaped = escaped[:200] + "..."
        items.append(f'<li style="margin-bottom:4pt;font-size:9pt;color:#666">{escaped}</li>')

    overflow = ""
    if len(deletions) > 30:
        overflow = f'<p style="font-size:9pt;color:#888">... and {len(deletions) - 30} more deleted sections.</p>'

    return f"""
        <div class="summary-legend" style="margin-top:16pt">
            <h3 style="color:#c0392b">Deleted text</h3>
            <p style="font-size:9pt;color:#888">The following text was present in the original but removed in the modified document:</p>
            <ul style="padding-left:16pt">{"".join(items)}</ul>
            {overflow}
        </div>
    """


def _render_structure_changes_html(summary: dict) -> str:
    """Render header/footer changes that are not always visible in Word diffs."""
    changes = summary.get("structure_changes") or []
    if not changes:
        return ""

    items = []
    for change in changes:
        text = _render_structure_change_summary(change)
        if text:
            items.append(f'<li style="margin-bottom:4pt;font-size:10pt;color:#555">{text}</li>')
    ...
    return f"""
        <div class="summary-legend" style="margin-top:16pt">
            <h3 style="color:#2c3e50">Header/footer changes</h3>
            <p style="font-size:10pt;color:#555">The following changes were detected outside the main document body:</p>
            <ul style="padding-left:16pt">{"".join(items)}</ul>
        </div>
    """


def _render_structure_change_summary(change: dict) -> str:
    """Render one header/footer finding with diff-style color markup."""
    ...
    if change_type == "added":
        prefix, sep, details = summary.partition(": ")
        if details:
            return f'{html_mod.escape(prefix + sep)}<span class="added">{html_mod.escape(details)}</span>'
        return f'<span class="added">{html_mod.escape(summary)}</span>'

    if change_type == "deleted":
        return f'<span class="deleted">{html_mod.escape(summary)}</span>'

    prefix, sep, details = summary.partition(": ")
    if not details:
        return html_mod.escape(summary)

    rendered_details = []
    for detail in details.split("; "):
        escaped = html_mod.escape(detail)
        lower = detail.lower()
        if " added" in lower or lower.startswith("text added") or lower.startswith("field added"):
            rendered_details.append(f'<span class="added">{escaped}</span>')
        elif " removed" in lower or lower.startswith("text removed") or lower.startswith("field removed"):
            rendered_details.append(f'<span class="deleted">{escaped}</span>')
        else:
            rendered_details.append(escaped)

    return html_mod.escape(prefix + sep) + "; ".join(rendered_details)


def _render_summary_pdf(summary: dict, original_name: str, modified_name: str, deletions: list | None = None) -> bytes:
...
<body>
    <div class="summary-page">
        ...
        {_render_structure_changes_html(summary)}
        {_render_deletions_html(deletions)}
        <div class="summary-footer">
...
def _merge_pdfs(
    ...
    """Merge the main document PDF and summary."""
    ...
    try:
        ...
        if docx_path:
            _write_docx(
                ...
                tmp_docx_path,
                doc_tree,
                extra_parts=summary.get("_extra_parts"),
            )
        else:
